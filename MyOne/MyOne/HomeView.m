...
@property (nonatomic, strong) KKImageView *paintImageView;
...
- (void)setUpViews {
	...
	// 初始化显示画控件
	self.paintImageView = [[KKImageView alloc] init];
...
//    self.monthAndYearLabel.font = [UIFont systemFontOfSize:10];
	...
	[self.monthAndYearLabel mas_makeConstraints:^(MASConstraintMaker *make) {
		make.top.equalTo(self.dayLabel.mas_bottom).with.offset(4);
		make.left.equalTo(self.containerView.mas_left).with.offset(10);
		make.width.mas_equalTo(@70);
...
- (void)configureViewWithHomeEntity:(HomeEntity *)homeEntity animated:(BOOL)animated {
	...
	[self.paintImageView configImageViewWithUrl:[NSURL URLWithString:homeEntity.strThumbnailUrl] animated:animated];
